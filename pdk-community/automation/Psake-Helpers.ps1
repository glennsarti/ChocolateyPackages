...
Function Invoke-SubmitMissingPackages($pkgDir,$locallist) {
  ...
  $script:ChocoPackageName | ForEach-Object {
    $pkgName = $_
    ...
    if ($chocoPkgList -eq $null) { $chocoPkgList = @() }
    if ($chocoPkgList.GetType().ToString() -eq 'System.String') { $chocoPkgList = @($chocoPkgList) }
    ...
    if ($localPkgList -eq $null) { $localPkgList = @() }
    if ($localPkgList.GetType().ToString() -eq 'System.String') { $localPkgList = @($localPkgList) }
