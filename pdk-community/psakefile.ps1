...
Task Clean_Choco_Install -Description 'Cleans out a failed Installations' {
  # if service delete it

  $installDir = 'C:\tools\' + $script:ChocoPackageName
  if (Test-Path -Path $installDir) {
    Write-Host "Purging $installDir ..."
    Remove-Item -Path $installDir -Recurse -Force -Confirm:$false | Out-Null
  }

  $installDir = 'C:\ProgramData\chocolatey\lib\' + $script:ChocoPackageName
  if (Test-Path -Path $installDir) {
    Write-Host "Purging $installDir ..."
    Remove-Item -Path $installDir -Recurse -Force -Confirm:$false | Out-Null
  }
...
Task Pack -Depends Clean -Description 'Packs a nuget template into a package' {
  ...
  if ($pkgname -eq '') {
    ...
    if ($pkgList.GetType().ToString() -eq 'System.String') { $pkgList = @($pkgList) }

    # Get the packagename
    do {
      $index = 1
      Write-Host "Select a package to pack"
      $pkgList | % {
        Write-Host "$($index). $($_)"
        $index++
      }
...
Task Automate_SourceTemplates -Description 'Creates package templates for new versions' {
...
Task CreateNewPackages -Description 'Updates the repository with newly release versions' {
